...
x = np.array(red_images + green_images + blue_images, dtype=np.uint8)
y = np.array([red_rbg for _ in range(data_size)] +
             [green_rbg for _ in range(data_size)] +
             [blue_rbg for _ in range(data_size)])
